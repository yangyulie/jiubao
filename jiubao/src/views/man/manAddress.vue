...
export default {
  ...
  methods: {
    ...
    goUseAddress(id){
      this.$router.push({
          path:'/manOrder?cId='+this.cId+'&cartIds='+this.cartIds+'&addressId='+ id+'&invoiceId='+ this.invoiceId,
          replace:true
      })
    },
    defaultAddress(status,id){
      ...
      ApiUser.defaultAddress(questData).then(res=>{
        Toast(res.msg)
        if(res.code==1){
          this.init();
        }
    ...
    getAddressFn(){//获取地址列表
        Api.getAddressList({cId:this.cId}).then(res=>{
            this.list= res.rows;
            this.datas = res;
        })
    },
